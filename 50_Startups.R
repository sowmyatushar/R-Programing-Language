...
Startups <- read.csv ("C:/Users/tussh/Documents/Multi Linear Regression/Startups.csv")
View(Startups)
attach(Startups)
...
model.car <- lm(Profit~R.D.Spend+Marketing.Spend)
model.car1 <- lm(Profit~., data = Startup)# lm(Y ~ X)
summary(model.car1)

model.carV<-lm(Profit~Administration)
summary(model.carV)

model.carW <- lm(Profit~Marketing.Spend)
summary(model.carW)

model.carVW <- lm(Profit~Administration+Marketing.Spend)
summary(model.carVW)
...
model.car1<-lm(Profit~ ., data=Startup[-50,-46])
summary(model.car1)
...
model.final1 <- lm(sqrt(Profit) ~ ., data=Startup[-50,-46])
